...
describe('ast.ts', () => {
  describe('Symbol table caching', () => {
    ...
    it('throws error when arity is missing for function/relation', () => {
      ...
      expect(() =>
        (add as any)(testSt, SymbolKind.Fun, Symbol('test'))
      ).to.throw("expected symbol of kind '2' to have an arity");
      expect(() =>
        (add as any)(testSt, SymbolKind.Rel, Symbol('test'))
      ).to.throw("expected symbol of kind '3' to have an arity");
    });
  ...
  describe('resolve()', () => {
    ...
    it('throws UnresolvedSymbolError for bad indices', () => {
      expect(() => resolve(SymbolKind.Const, 42, st)).to.throw(
        UnresolvedSymbolError
      );
    });
  });
  ...
  describe('construct()', () => {
    ...
    it('creates quantified formulas', () => {
      const formula = construct(st, ({ var: v, atom: a, forall, exists }) => {
        ...
        return forall([xSym.symbol], exists([ySym.symbol], innerFormula));
      });
    ...
    it('creates the same complex formula as in render test', () => {
      ...
      const formula = construct(
        ...
        ({ var: v, const: c, func: f, atom: a, forall, exists, implies }) => {
          ...
          return forall(
            [xSym.symbol],
            implies(RxfxgC, exists([ySym.symbol], SgY))
          );
        }
      );
